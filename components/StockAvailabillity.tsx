...
const StockAvailabillity = ({ stock, inStock,quantity } : { stock: number, inStock: number,quantity:number }) => {
  return (<>
    <p className='text-xl flex gap-x-2 max-[500px]:justify-center'>Availability: 
    {quantity>0 ? <span className='text-success flex items-center gap-x-1'>In stock <FaCheck /></span> :  <span className='text-error flex items-center gap-x-1'>Out of stock <FaXmark /></span>} 
    </p>
  </>
  )
}
